...
bool YOLO_DETECTION::check_output_dims(){
    for(int i = 0; i < m_OtpNum - 1 ; i++ ){//minus 1,因为最后一个是grid_anchors
        if(m_InpSp.h / m_strides[i] != m_OutEleDims[i][2]) {
            
            printf("m_InpSp.h / m_strides[i] = %d, m_OutEleDims[i][2] = %d not equals\n", \
                m_InpSp.h / m_strides[i], m_OutEleDims[i][2] );
            return false;
        }
        if(m_InpSp.w / m_strides[i] != m_OutEleDims[i][1]) {
            printf("m_InpSp.w / m_strides[i] = %d, m_OutEleDims[i][1] = %d not equals\n", \
            m_InpSp.w / m_strides[i], m_OutEleDims[i][1] );
            return false;
        }
    }
    return true;
...
ucloud::RET_CODE YOLO_DETECTION::postprocess(std::vector<float*> &output_datas, VecObjBBox &bboxes, std::vector<float> &aspect_ratios){
    ...
    rknn_output_to_boxes(output_datas, vecBox);
...
ucloud::RET_CODE YOLO_DETECTION::rknn_output_to_boxes( std::vector<float*> &output_datas, std::vector<ucloud::VecObjBBox> &bboxes){
    LOGI << "<- YOLO_DETECTION::rknn_output_to_boxes";
    int NC = m_clss.size();
    int NL = m_nl;//3
    int NA = 3;
    int D = NC + 5;
    ...
    float* anchor_grid = output_datas[NA];
            ...
            for(int h = 0; h < H; h++){//feature map h
                for(int w=0; w < W; w++){ //w
                    ...
                    if( objectness<m_threshold ) {
                        ...
                        cx = (cx*2 - 0.5 + cw )*m_strides[nl];
                        cy = (cy*2 - 0.5 + ch )*m_strides[nl];
                        cw = (cw*cw*4)* get_anchor_grid_from_c_data_layer(anchor_grid, nl, na , 0 , NA, NL);   //anchor_grid[nl*NL*NA*2+na*NA*2+0];
                        ch = (ch*ch*4)* get_anchor_grid_from_c_data_layer(anchor_grid, nl, na , 1 , NA, NL);   //anchor_grid[nl*NL*NA*2+na*NA*2+1];
                        fbox.x0 = cx - cw/2;
                        fbox.y0 = cy - ch/2;
                        fbox.x1 = cx + cw/2;
                        fbox.y1 = cy + ch/2;
